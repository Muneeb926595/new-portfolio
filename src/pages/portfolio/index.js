...
export const Portfolio = () => {
  ...
  useEffect(() => {
    if (currentSelection === 'all') {
      ...
      const newData = dataportfolio?.filter(item => item?.type?.toLowerCase()?.trim() === currentSelection?.toLowerCase()?.trim())
        ...
        <div style={{ display: 'flex', alignItems: 'center', }}>
          {
            projectTypes?.map((item, index) =>
              <h5 key={index} onClick={() => { setCurrentSelection(item) }} style={{ padding: '1rem', paddingLeft: "0px", textDecorationLine: currentSelection === item ? 'underline' : 'none', }}>{capitalizeWords(item)}</h5>
            )
          }
        </div>
